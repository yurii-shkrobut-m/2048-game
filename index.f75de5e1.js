...
function hasAdjacentTiles() {
    for(let i = 0; i < rowsNum; i++)for(let j = 0; j < columnsNum; j++){
...
function updateTable() {
    ...
    for(let i = 0; i < field.length; i++)for(let k = 0; k < field[i].length; k++){
...
function slide(row) {
    ...
    for(let i = 0; i < copy.length - 1; i++)if (copy[i] === copy[i + 1]) {
        copy[i] *= 2;
        copy[i + 1] = 0;
        score += copy[i];
    }
...
function fieldCombine(cell, num, row) {
    for(let r = 0; r <= num; r++)field[r][cell] = row[r];
}
function slideUp() {
